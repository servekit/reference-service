// Package pkg is the public surface of reference-service. It wires internal pieces
// into a runnable gRPC server, a gRPC client, and an
// in-process module entry point. Downstream services that depend on reference-service should import
...
// Server wraps a gRPC server for the reference service.
//
// Holds grpcSrv (the gRPC transport) and hdl (the
// Handler, which itself wraps the underlying *service.Service and exposes
// Start/Stop). There's no separate svc field — Handler is the single handle
// for both RPC dispatch and lifecycle.
//
// reference-service is gRPC-only: no HTTP gateway runs in-process. The
...
//     rules declared in reference.proto
func NewServer(cfg *config.Config, opts ...ServerOption) (*Server, error) {
	...
	grpcSrv := grpcx.New(
		...
		func(gs *grpc.Server) {
			referencev1.RegisterReferenceServiceServer(gs, hdl)
		},
		nil, // no HTTP gateway — gRPC-only service (scaffolded without --http)
